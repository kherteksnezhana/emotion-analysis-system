...
ИСПРАВЛЕНИЯ:
1. Теперь показывает реальные эмоции от модели, а не предугаданные
2. Эмодзи соответствуют реальному анализу модели
3. Вывод идёт ПОСЛЕ сохранения результата модели
"""
...
def generate_reports():
    ...
    for username, scenario in EMOTIONAL_PROFILES.items():
        # Пропускаем, если пользователя нет в списке USERS
        ...
        # Для хранения результатов анализа (чтобы потом вывести)
        analysis_results = []
        ...
        for day_idx in range(20):
            ...
            # Получаем целевое эмоциональное состояние для этого дня (для генерации текста)
            target_emotion = get_emotional_state(profile_type, day_idx, 20)
            
            # Генерируем текст отчёта в зависимости от отдела
            if department == "IT":
                text = generate_report_for_it(target_emotion)
            elif department == "Маркетинг":
                text = generate_report_for_marketing(target_emotion)
            elif department == "Продажи":
                text = generate_report_for_sales(target_emotion)
            elif department == "Бухгалтерия":
                text = generate_report_for_accounting(target_emotion)
            else:
            ...
            if report_id:
                ...
                # Реальный анализ эмоций моделью
                analysis = analyze_emotion(text)
                analysis_results.append((report_id, analysis))
        ...
        # Обновляем даты и выводим результаты
        for i, report_id in enumerate(report_ids):
            if i < len(working_dates):
                report_date = working_dates[i]
                update_report_date(report_id, report_date)
                
                # Берём реальный результат анализа
                if i < len(analysis_results):
                    _, analysis = analysis_results[i]
                    emotion_label = analysis['label']
                    
                    # Иконка по реальному результату модели
                    ...
                    print(f"    [{i+1:2d}] {report_date.strftime('%d.%m.%Y')} {emotion_icon} {analysis['display_label']} "
                          f"(уверенность: {analysis['score']:.2f}, выгорание: {analysis['burnout_index']:.2f})")
